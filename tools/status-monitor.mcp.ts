...
const server = new McpServer({
    ...
    version: '1.0.0',
    description:
        'Interval-based status monitoring with GitHub API integration for project management.'
});
...
server.tool(
    'create_monitor',
    {
        name: z
            .string()
            .describe('Human-readable name for the monitoring task'),
        interval: z
            .number()
            .min(1000)
            .describe('Interval in milliseconds (minimum 1 second)'),
        command: z
            .string()
            .describe(
                "Command to execute for status check (e.g., 'gh pr list --state open')"
            )
    },
            ...
            content: [
                {
                    type: 'text',
                    text:
                        `✅ Monitor '${name}' created with ID: ${id}\n` +
...
server.tool(
    'list_monitors',
    {
        activeOnly: z
            .boolean()
            .optional()
            .describe('Show only active monitors (default: true)')
    },
...
server.tool('check_due_monitors', {}, async () => {
    const now = Date.now();
    const dueTasks = db
        .prepare(
            `
...
server.tool(
    'monitor_history',
    {
        ...
        limit: z
            ...
            .optional()
            .default(10)
            .describe('Number of recent executions to show')
    },
    async ({ taskId, limit }) => {
        const executions = db
            .prepare(
        ...
        const task = db
            .prepare('SELECT name FROM monitor_tasks WHERE id = ?')
            .get(taskId) as any;
...
server.tool(
    'toggle_monitor',
    {
        ...
        active: z
            .boolean()
            .describe('Set monitor active (true) or inactive (false)')
    },
    async ({ taskId, active }) => {
        const result = db
            .prepare(
...
async function main() {
    ...
    console.log(
        'Status Monitor MCP Server started and listening via StdioServerTransport.'
    );
}
