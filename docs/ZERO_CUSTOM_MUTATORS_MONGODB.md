...
export class MongoDatabase implements Database<MongoTransaction> {
    async transaction<R>(
        callback: (tx: MongoTransaction, hooks: TransactionProviderHooks) => Promise<R>,
...
Test services with mocked sessions:
...
describe('PushController', () => {
    it('should process message.send mutation', async () => {
        ...
        expect(response.body.mutationResults).toBeDefined();
    });
});
```
