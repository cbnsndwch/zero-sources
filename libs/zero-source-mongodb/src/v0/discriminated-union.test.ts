...
describe('Discriminated Union Utilities', () => {
    describe('matchesFilter', () => {
        ...
        it('should match $ne operator', () => {
            expect(matchesFilter(sampleDoc, { archived: { $ne: true } })).toBe(
                true
            );
            expect(matchesFilter(sampleDoc, { archived: { $ne: false } })).toBe(
                false
            );
        });

        it('should match $in operator', () => {
            expect(matchesFilter(sampleDoc, { t: { $in: ['d', 'p'] } })).toBe(
                true
            );
            expect(matchesFilter(sampleDoc, { t: { $in: ['c', 'p'] } })).toBe(
                false
            );
        });
        ...
        it('should match nested properties', () => {
            expect(
                matchesFilter(sampleDoc, { 'nested.property': 'value' })
            ).toBe(true);
            expect(
                matchesFilter(sampleDoc, { 'nested.property': 'other' })
            ).toBe(false);
        });
