...
import * as auth41 from '../auth-41.js';

import * as ClientConstants from '../constants/client.js';
import CharsetToEncoding from '../constants/charset-encodings.js';

import type Connection from '../connection.js';

import Command from './command.base.js';
import { SSLRequestPacket } from 'src/packets/ssl-request.packet.js';
import { IPacket } from 'src/contracts/packet.js';

function flagNames(flags: number) {
    const res = [];

    for (const c in ClientConstants) {
        if (flags & ClientConstants[c as keyof typeof ClientConstants]) {
...
export default class ClientHandshake extends Command {
    ...
    sendSSLRequest(connection: Connection) {
        const sslRequest = new SSLRequestPacket(
            this.clientFlags,
    ...
    handshakeInit(helloPacket?: IPacket, connection?: Connection) {
        if (connection) {
            this.on('error', e => {
        ...
        this.handshake = ClientHandshake.fromPacket(helloPacket);
        if (connection.config.debug) {
        ...
        connection.serverEncoding =
            CharsetToEncoding[this.handshake.characterSet];
    ...
    handshakeResult(packet, connection) {
        ...
        if (marker === 0xfe || marker === 1 || marker === 0x02) {
            ...
            try {
                if (marker === 1) {
                    authSwitch.authSwitchRequestMoreData(
                        packet,
                        connection,
                        this
                    );
