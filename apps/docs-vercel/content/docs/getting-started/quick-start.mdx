...
```typescript title="src/schema/message.schema.ts" {3-12}
...
```json title="config/table-mappings.json" {2-10}
{
    "message": {
        "collection": "messages",
        "fields": {
...
```typescript title="client.ts" {4-7,10-11}
...
```javascript title="MongoDB Shell" {1-5}
