...
import { ChevronDown, ExternalLink, icons } from 'lucide-react';
import {
    type ComponentProps,
    createContext,
    type FC,
...
/**
 * Resolves an icon prop to a ReactNode
 * If the icon is a string (lucide icon name), it will look up and render the icon from lucide-react
 * Otherwise returns the icon as-is
 */
...
export function SidebarItem({
    ...
    return (
        <Link
            ...
            prefetch={prefetch}
        >
            {resolvedIcon ?? (props.external ? <ExternalLink /> : null)}
            {props.children}
        </Link>
    );
}
...
function PageTreeFolder({
    ...
    const resolvedIcon = useMemo(() => resolveIcon(item.icon), [item.icon]);
