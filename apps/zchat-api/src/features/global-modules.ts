...
import { ZeroMongoModule, ZeroMongoModuleOptions } from '@cbnsndwch/zero-source-mongodb';
...
const zqliteWatermarkModule = ZqliteWatermarkModule.forRootAsync({
    ...
    async useFactory(config: ConfigService<AppConfig>) {
        ...
        invariant(typeof zeroConfig === 'object', 'Invalid zero config, expected object');

        const { kv } = zeroConfig;
        invariant(typeof kv === 'object', 'Invalid zero.kv config, expected object');

        const { provider, zqlite } = kv;
        invariant(provider === 'zqlite', 'Invalid KV provider, expected `zqlite`');
        invariant(
            typeof zqlite === 'object' && typeof zqlite.file === 'string' && zqlite.file.length > 0,
...
const zeroChangeSourceModule = ZeroMongoModule.forRootAsync({
    ...
    async useFactory(config: ConfigService<AppConfig>) {
        ...
        invariant(typeof auth === 'object', 'Invalid zero.auth config, expected object');
