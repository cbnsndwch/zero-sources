...
describe('ChatInput', () => {
  ...
  it('renders with basic textarea by default', () => {
    ...
    expect(screen.queryByTestId('rich-message-editor')).not.toBeInTheDocument();
  });
  ...
  it('renders with RichMessageEditor when useRichEditor is true', () => {
    ...
    expect(screen.getByTestId('rich-editor-placeholder')).toHaveTextContent('Type a message...');
    expect(screen.getByTestId('rich-editor-max-length')).toHaveTextContent('1000');
  ...
  it('applies correct room type and id props', () => {
    render(<ChatInput roomId="my-room" roomType="group" useRichEditor={true} />);
