...
class PerformanceMonitor {
    ...
    recordInitialization(startTime: number): void {
        ...
        if (process.env.NODE_ENV === 'development') {
            ...
            if (duration > 100) {
                console.warn(`⚠️ Editor initialization exceeded 100ms target (${duration.toFixed(2)}ms)`);
            }
        }
    }
    ...
    recordKeystroke(startTime: number): void {
        ...
        if (process.env.NODE_ENV === 'development' && latency > 16) {
            console.warn(`⚠️ Keystroke latency exceeded 16ms target (${latency.toFixed(2)}ms)`);
        }
    }
    ...
    recordSerialization(startTime: number, contentLength?: number): void {
        ...
        if (process.env.NODE_ENV === 'development') {
            if (duration > 50) {
                console.warn(`⚠️ Serialization exceeded 50ms target (${duration.toFixed(2)}ms)`);
            }
        }
    }
    ...
    recordDeserialization(startTime: number, contentLength?: number): void {
        ...
        if (process.env.NODE_ENV === 'development') {
            if (duration > 50) {
                console.warn(`⚠️ Deserialization exceeded 50ms target (${duration.toFixed(2)}ms)`);
            }
        }
    }
    ...
    recordUndoRedo(startTime: number, operation: 'undo' | 'redo'): void {
        ...
        if (process.env.NODE_ENV === 'development') {
            if (duration > 100) {
                console.warn(`⚠️ ${operation} operation exceeded 100ms target (${duration.toFixed(2)}ms)`);
            }
        }
    }
    ...
    getAverageKeystrokeLatency(): number {
        ...
        return this.keystrokeBuffer.reduce((a, b) => a + b, 0) / this.keystrokeBuffer.length;
    }
    ...
    private addEntry(metric: keyof EditorPerformanceMetrics, value: number, metadata?: Record<string, any>): void {
        this.entries.push({
            timestamp: Date.now(),
            metric,
            value,
...
export function usePerformanceMonitor() {
    return {
        ...
        recordInitialization: performanceMonitor.recordInitialization.bind(performanceMonitor),
        recordKeystroke: performanceMonitor.recordKeystroke.bind(performanceMonitor),
        recordSerialization: performanceMonitor.recordSerialization.bind(performanceMonitor),
        recordDeserialization: performanceMonitor.recordDeserialization.bind(performanceMonitor),
        recordUndoRedo: performanceMonitor.recordUndoRedo.bind(performanceMonitor),
    ...
    startMonitoring(intervalMs: number = 30000): void {
        ...
        this.checkInterval = setInterval(() => {
            ...
            if (increase > this.criticalThreshold) {
                console.error(`🚨 Critical memory leak detected: ${(increase / 1024 / 1024).toFixed(2)}MB increase`);
            } else if (increase > this.warningThreshold) {
                console.warn(`⚠️ Potential memory leak: ${(increase / 1024 / 1024).toFixed(2)}MB increase`);
            }
