...
function defaultSanitizeHtml(html: string): string {
    ...
    allElements.forEach(el => {
        ...
        if (el.hasAttribute('style')) {
            ...
            const safeStyle = style.replace(/(javascript:|expression\(|@import|behavior:)/gi, '');
...
function handlePasteCommand(
    ...
    // Check for forced plain text paste (Ctrl+Shift+V)
    const forcePlainText = event.shiftKey && (event.ctrlKey || event.metaKey);
    ...
    if (!forcePlainText && htmlContent && preserveFormatting) {
    ...
    if (maxPasteLength && contentLength > maxPasteLength) {
        console.warn(`Paste content exceeds maximum length of ${maxPasteLength} characters`);
