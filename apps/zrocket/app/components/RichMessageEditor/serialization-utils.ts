 ...
 */
export function validateSerializedEditorState(state: SerializedEditorState): boolean {
  try {
    // Check root structure
    if (!state.root) {
      console.error('SerializedEditorState validation failed: missing root');
      return false;
    }
    ...
    if (root.type !== 'root') {
      console.error('SerializedEditorState validation failed: root.type is not "root"');
      return false;
    }

    if (typeof root.version !== 'number') {
      console.error('SerializedEditorState validation failed: root.version is not a number');
      return false;
    }

    if (typeof root.format !== 'string') {
      console.error('SerializedEditorState validation failed: root.format is not a string');
      return false;
    }

    if (typeof root.indent !== 'number') {
      console.error('SerializedEditorState validation failed: root.indent is not a number');
      return false;
    }

    if (root.direction !== null && root.direction !== 'ltr' && root.direction !== 'rtl') {
      console.error('SerializedEditorState validation failed: root.direction is not valid');
      return false;
    }

    // Validate children array
    if (!Array.isArray(root.children)) {
      console.error('SerializedEditorState validation failed: root.children is not an array');
      return false;
    }
    ...
    console.error('SerializedEditorState validation failed with error:', error);
    return false;
  }
}
 ...
 */
export function ensureValidSerializedEditorState(state: SerializedEditorState): SerializedEditorState {
  ...
  normalizedState.root = {
    ...
    children: Array.isArray(normalizedState.root.children) ? normalizedState.root.children : []
  };

  // Process each child to ensure proper format
  normalizedState.root.children = normalizedState.root.children.map(child => {
    ...
    return {
      ...
      children: Array.isArray(child.children) ? child.children.map(textNode => {
        ...
        return {
          ...textNode,
          text: textNode.text || '',
          format: typeof textNode.format === 'number' ? textNode.format : 0,
