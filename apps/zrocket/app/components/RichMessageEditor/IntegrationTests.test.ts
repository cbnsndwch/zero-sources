...
describe('RichMessageEditor - Complete Integration Scenarios', () => {
    describe('Message Database Scenarios', () => {
        it('should handle message data as it would come from database', () => {
            ...
            expect(validateSerializedEditorState(messageFromDB.contents)).toBe(
                true
            );

            // Ensure it can be safely used for editing
            const editableContent = ensureValidSerializedEditorState(
                messageFromDB.contents
            );
        ...
        it('should handle legacy message formats and normalize them', () => {
            ...
            const normalizedContent = ensureValidSerializedEditorState(
                legacyMessage.contents as any
            );
        ...
        it('should handle empty/null messages from database', () => {
            ...
            emptyMessages.forEach((emptyMessage, index) => {
                const normalized = ensureValidSerializedEditorState(
                    emptyMessage as any
                );
                expect(
                    validateSerializedEditorState(normalized),
                    `Failed for empty message at index ${index}`
                ).toBe(true);
            ...
            const complexMessage = {
                root: {
                    children: [
                        ...
                        {
                            children: [{ text: '', format: 0, type: 'text' }],
                            direction: 'ltr',
                            format: '',
                            indent: 0,
                            type: 'paragraph',
