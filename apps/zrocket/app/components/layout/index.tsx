...
export default function SidebarLayout() {
  ...
  useEffect(() => {
    ...
    if (pathParts[1]) {
      const currentType = pathParts[1] === 'c' ? 'channels' : 
                         pathParts[1] === 'p' ? 'groups' : 
                         pathParts[1] === 'd' ? 'dms' : 'channels';
  ...
  const handleRoomTypeChange = (newRoomType: string) => {
    ...
    const roomsForType = roomsByType[newRoomType as keyof typeof roomsByType];
    ...
    if (lastVisited) {
      const foundRoom = roomsForType.find(room => room.id === lastVisited);
      if (foundRoom) {
  ...
  useEffect(() => {
    if (location.pathname === '/') {
      ...
      if (lastVisited) {
        const foundRoom = channelsForType.find(room => room.id === lastVisited);
        if (foundRoom) {
              ...
              <ResizablePanelGroup direction="horizontal" className="flex-1">
                ...
                <ResizablePanel defaultSize={20} minSize={15} maxSize={35}>
                  <div className="h-full border-r border-border bg-background overflow-hidden">
                    <RoomList roomType={activeRoomType} searchQuery={searchQuery} />
                  </div>
                </ResizablePanel>
                ...
                {/* Chat content panel */}
                <ResizablePanel defaultSize={isRoomDetailsOpen ? 55 : 80} minSize={30}>
                  <main className="h-full overflow-hidden">
                    {params.chatId || params.groupId || params.channelId ? (
                      <div className="h-full">
                        <Outlet context={{ isRoomDetailsOpen, setIsRoomDetailsOpen }} />
                      </div>
                    ) : (
                      <EmptyChat roomType={activeRoomType} />
                    )}
                  </main>
                </ResizablePanel>
