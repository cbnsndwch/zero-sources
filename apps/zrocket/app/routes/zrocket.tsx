...
export default function ZRocketDemo() {
    ...
    const seedData = async () => {
        ...
        try {
            ...
            setSeedStatus(result.success ? 'Data seeded successfully!' : `Error: ${result.message}`);
        } catch (error) {
        ...
        return (
            <div className="min-h-screen bg-gray-50 flex items-center justify-center">
                <div className="text-center">
                    ...
                    <p className="mt-4 text-gray-600">Loading ZRocket Demo...</p>
                    <p className="text-sm text-gray-500 mt-2">Make sure the ZRocket API server is running on port 8012</p>
                </div>
            </div>
        );
    }
        ...
        <div className="min-h-screen bg-gray-50 py-8">
            <div className="max-w-6xl mx-auto px-4">
                ...
                <div className="text-center mb-8">
                    <h1 className="text-4xl font-bold text-gray-900 mb-2">🚀 {demoInfo.title}</h1>
                    <p className="text-xl text-gray-600 max-w-3xl mx-auto">{demoInfo.description}</p>
                </div>
                ...
                <div className="bg-white rounded-lg shadow-md p-6 mb-8">
                    ...
                    <p className="text-gray-600 mb-4">
                        Seed the MongoDB database with sample data to see discriminated union tables in action.
                    </p>
                    <button
                        onClick={seedData}
                        disabled={loading}
                    ...
                    {seedStatus && (
                        <p className={`mt-3 ${seedStatus.includes('Error') ? 'text-red-600' : 'text-green-600'}`}>
                            {seedStatus}
                        </p>
                    )}
                </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
                    {Object.entries(demoInfo.tables).map(([sourceType, tables]: [string, any]) => (
                        <div key={sourceType} className="bg-white rounded-lg shadow-md p-6">
                            <h3 className="text-xl font-semibold mb-4 text-center">
                                {sourceType === 'fromRoomsCollection' && '🏠 Rooms Collection'}
                                {sourceType === 'fromMessagesCollection' && '💬 Messages Collection'}
                                {sourceType === 'fromParticipantsCollection' && '👥 Participants Collection'}
                            </h3>
                ...
                <div className="bg-white rounded-lg shadow-md p-6 mb-8">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {demoInfo.features.map((feature: string, index: number) => (
                ...
                <div className="bg-white rounded-lg shadow-md p-6">
                    ...
                    <div className="space-y-2">
                        {demoInfo.endpoints.map((endpoint: string, index: number) => (
